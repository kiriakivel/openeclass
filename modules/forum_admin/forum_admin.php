...
if(isset($forumgo)) {
	...
	if ($result and mysql_num_rows($result) > 0) {
		$tool_content .= "<form action=\"". htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'). "?forumgoadd=yes&ctg=$ctg&cat_id=$cat_id\" method=post>
		...
		while(list($forum_id, $forum_name, $forum_desc, $forum_access,
			$forum_moderator, $forum_type) = mysql_fetch_row($result)) {
				...
				$tool_content .= "\n<td align='center'>
				<a href='". htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'). "?forumgoedit=yes&forum_id=$forum_id&ctg=$ctg&cat_id=$cat_id'>
				<img src='../../template/classic/img/edit.gif' title='$langModify' border='0'></img></a>&nbsp;
				<a href='". htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'). "?forumgodel=yes&forum_id=$forum_id&cat_id=$cat_id&ctg=$ctg&ok=0' onClick='return confirmation();'>
		...
		$tool_content .= "
		<form action=\"". htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'). "?forumgoadd=yes&ctg=$ctg&cat_id=$cat_id\" method=post onsubmit=\"return checkrequired(this,'forum_name');\">
		...
		<div align=\"right\"><a href=\"". htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'). "?forumadmin=yes\">$langBackCat</a></div>";
	}
	// forum go edit
	elseif(isset($forumgoedit)) {
		...
		$tool_content .= "
		<form action=\"". htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'). "?forumgosave=yes&ctg=$ctg&cat_id=".@$cat_id."\" method=post onsubmit=\"return checkrequired(this,'forum_name');\">
	...
	elseif(isset($forumcatedit)) {
		...
		$tool_content .= "
  		<form action='". htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'). "?forumcatsave=yes' method=post onsubmit=\"return checkrequired(this,'cat_title');\">
    		<input type=hidden name=cat_id value=$cat_id>
    		<table width=99% class=\"FormData\">
	...
	elseif (isset($forumcatsave)) {
		...
		$tool_content .= "\n<p class=\"success_small\">$langNameCatMod<br /><a href=\"". htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'). "?forumadmin=yes\">$langBack</a></p>";
	}
	...
	elseif(isset($forumgosave)) {
		...
		<a href=\"". htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'). "?forumgo=yes&cat_id=$cat_id&ctg=$ctg\">$langBack</a></p>";
	}
	...
	elseif(isset($forumcatadd)) {
		...
		<a href='". htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'). "?forumadmin=yes'>$langBack</a></p>";
		}
	...
	elseif(isset($forumgodel)) {
		...
		$tool_content .= "\n<p class=\"success_small\">$langForumDelete<br />
			<a href=\"". htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'). "?forumgo=yes&ctg=$ctg&cat_id=$cat_id\">$langBack</a></p>";
	} else {
		...
		$tool_content .= "<form action=\"". htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'). "?forumadmin=yes\" method=post></td><tr><td>";
		...
		while(list($cat_id, $cat_title) = mysql_fetch_row($result)) {
			...
			<a href='". htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'). "?forumcatedit=yes&cat_id=$cat_id'>
			<img src='../../template/classic/img/edit.gif' border='0' title='$langModify'></img></a>&nbsp;
			<a href='". htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'). "?forumcatdel=yes&cat_id=$cat_id&ok=0' onClick='return confirmation();'>
			<img src='../../template/classic/img/delete.gif' border='0' title='$langDelete'></img></a>
			<a href='". htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'). "?forumcatnotify=$link_notify&cat_id=$cat_id'>	
			<img src='../../template/classic/img/announcements$icon.gif' border='0' title='$langNotify'></img></a>
			</td></tr>";
			$i++;
		}
		$tool_content .= "</tbody></table></form><br/>
		<form action=\"". htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'). "?forumcatadd=yes\" method=post onsubmit=\"return checkrequired(this,'catagories');\">
		<table width=99% class=\"FormData\" align=\"left\">
		<tbody><tr>
		<th width='220'>&nbsp;</th>
