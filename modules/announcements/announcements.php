...
hContent;
    ...
    $tool_content .= "
      <div id='operations_container'>
        <ul id='opslist'>
          <li><a href='" . htmlspecialchars($_SERVER[PHP_SELF], ENT_QUOTES, 'UTF-8') . "?addAnnouce=1'>" . $langAddAnn . "</a></li>";

    if ($announcementNumber > 1 || isset($_POST['submitAnnouncement'])) {
        $tool_content .= "
          <li><a href='htmlspecialchars($_SERVER[PHP_SELF], ENT_QUOTES, 'UTF-8')?deleteAllAnnouncement=1' onClick='return confirmation('all');'>$langEmptyAnn</a></li>";
    }
    $tool_content .= "
        </ul>
      </div>";
    ...
    if ($displayForm and
        ...
        $tool_content .= "<form method='post' action='htmlspecialchars($_SERVER[PHP_SELF], ENT_QUOTES, 'UTF-8')' onsubmit=\"return checkrequired(this, 'antitle');\">";
	...
	while ($myrow = mysql_fetch_array($result)) {
        ...
        <a href='htmlspecialchars($_SERVER[PHP_SELF], ENT_QUOTES, 'UTF-8')?modify=" . $myrow['id'] . "'>
        <img src='../../template/classic/img/edit.gif' title='" . $langModify . "' /></a>
        <a href='htmlspecialchars($_SERVER[PHP_SELF], ENT_QUOTES, 'UTF-8')?delete=" . $myrow['id'] . "' onClick=\"return confirmation('');\">
	...
	if ($iterator != 1)  {
		$tool_content .= "<a href='htmlspecialchars($_SERVER[PHP_SELF], ENT_QUOTES, 'UTF-8')?up=" . $myrow["id"] . "'><img class='displayed' src='../../template/classic/img/up.gif' title='" . $langUp . "' /></a>";
	}
        // DISPLAY MOVE DOWN COMMAND
	if ($iterator < $bottomAnnouncement) {
		$tool_content .= "<a href='htmlspecialchars($_SERVER[PHP_SELF], ENT_QUOTES, 'UTF-8')?down=" . $myrow["id"] . "'><img class='displayed' src='../../template/classic/img/down.gif' title='" . $langDown . "' /></a>";
	}
	if ($announcementNumber > 1) {
		$tool_content .= "</td>";
	}
