...
$topic_count = mysql_fetch_row(db_query("SELECT COUNT(*) FROM topics WHERE forum_id = '". escapeSimple($forum). "'"));
...
if(isset($topicnotify)) { // modify topic notification
	$rows = mysql_num_rows(db_query("SELECT * FROM forum_notify 
		WHERE user_id = '". escapeSimple($uid). "' AND topic_id = '". escapeSimple($topic_id). "' AND course_id = '". escapeSimple($cours_id). "'", $mysqlMainDb));
	if ($rows > 0) {
		db_query("UPDATE forum_notify SET notify_sent = '$topicnotify' 
			WHERE user_id = '". escapeSimple($uid). "' AND topic_id = '". escapeSimple($topic_id). "' AND course_id = '". escapeSimple($cours_id). "'", $mysqlMainDb);
	} else {
		db_query("INSERT INTO forum_notify SET user_id = $uid,
		topic_id = '". escapeSimple($topic_id). "', notify_sent = 1, course_id = '". escapeSimple($cours_id). "'", $mysqlMainDb);
	}
}
...
$sql = "SELECT t.*, p.post_time, p.nom AS nom1, p.prenom AS prenom1
        ...
        WHERE t.forum_id = '". escapeSimple($forum). "' 
...
if (mysql_num_rows($result) > 0) { // topics found
	while($myrow = mysql_fetch_array($result)) {
		...
		list($topic_action_notify) = mysql_fetch_row(db_query("SELECT notify_sent FROM forum_notify 
			WHERE user_id = '". escapeSimple($uid). "' AND topic_id = $myrow[topic_id] AND course_id = '". escapeSimple($cours_id). "'", $mysqlMainDb));
		...
		if (isset($_GET['start']) and $_GET['start'] > 0) {
			$tool_content .= "<a href='". htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'). "?forum=$forum&start=$_GET[start]&amp;topicnotify=$topic_link_notify&amp;topic_id=$myrow[topic_id]'>
			<img src='../../template/classic/img/announcements$topic_icon.gif' title='$langNotify'></img></a>";
		} else {
			$tool_content .= "<a href='". htmlspecialchars($_SERVER['PHP_SELF'], ENT_QUOTES, 'UTF-8'). "?forum=$forum&amp;topicnotify=$topic_link_notify&amp;topic_id=$myrow[topic_id]'>
			<img src='../../template/classic/img/announcements$topic_icon.gif' title='$langNotify'></img></a>";
		}
		$tool_content .= "</td></tr>";
	} // end of while
