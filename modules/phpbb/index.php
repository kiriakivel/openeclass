...
if ($total_categories) {
	...
	for($i=0; $i < $total_categories; $i++) {
		...
		$tool_content .= "<tr><td colspan='5' class='Forum'>&nbsp;$title</td>
			...
			<a href='htmlspecialchars($_SERVER[PHP_SELF], ENT_QUOTES, 'UTF-8')?forumcatnotify=$link_notify&amp;cat_id=$catNum'>	
		...
		for ($x=0; $x < count($forum_row); $x++) {
			...
			if ($forum_row[$x]["cat_id"] == $categories[$i]["cat_id"]) {
				...
				$tool_content .= "<td class='Forum_leftside' style='text-align:center'>
					<a href='htmlspecialchars($_SERVER[PHP_SELF], ENT_QUOTES, 'UTF-8')?forumnotify=$forum_link_notify&amp;forum_id=$forum'>	
