...
function display_monthcalendar($agendaitems, $month, $year, $weekdaynames, $monthName, $langToday) {
	...
	$backwardsURL = htmlspecialchars($_SERVER[PHP_SELF], ENT_QUOTES, 'UTF-8')."?month=".($month==1 ? 12 : $month-1)."&year=".($month==1 ? $year-1 : $year);
	$forewardsURL = htmlspecialchars($_SERVER[PHP_SELF], ENT_QUOTES, 'UTF-8')."?month=".($month==12 ? 1 : $month+1)."&year=".($month==12 ? $year+1 : $year);
